 ...
 <div class="row hidemsg" >
  ...
  <div class="x_panel">
    ...
    <div class="x_content"> <br>
      <form class="form-horizontal" method="POST" action="{{ url('product/edit',array($result->id)) }}" enctype="multipart/form-data">
        {{ csrf_field() }}
            ...
            <div class="row">
              ...
              <div class="col-md-12">
                <select class="form-control" id="brand" name="brand" style="width: 100%">
                   @foreach($brands as $d)
                   <option value="{{ $d->id }}" {{ ($d->id == $result->brand)?'selected=selected':'' }} >{{ $d->name }}</option>
                  @endforeach
              </select>
              </div>
            </div>
     ...
     <div class="row">
          ...
          <div class="col-md-6">
            <div class="row">
              ...
              <div class="col-md-12">
              <input type="text" name="sku" class="form-control " value="{{ $result->sku }}" required>
              </div>
            </div>
          </div>
        </div>
